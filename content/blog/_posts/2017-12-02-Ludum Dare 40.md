...
- [ ] The map file has be a simple array with the Tile Type
...
- [x] Board support for `search(type)` to be able to get a list of `Tiles` or `Pieces`
- [x] Board support for `search(type, tag)` to be able to search for specifically tagged pieces/tiles
- [x] Board support for `search(type, tag, row, col)` to restrict search to a single tile, row, or column
- [x] Board support for `search(type, tag, row, col, distance)` to restrich search to a rectangular zone around tile
- [x] Make TheCad move by querying the Board for the Tile next to him, and properly attaching to the tile
