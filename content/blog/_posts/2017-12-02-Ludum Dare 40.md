...
## Status: Compo Failed. Jam Salvaged!

I am making an "Simultaneous Dates Simulator" where you try to go on as many possible simultaneous dates in the same restaurant without getting caught. "The More Dates You Have, the Worse It Gets" after all!

Here is the Jam submission, as of T+65:11 Hours! Play it [HERE]({{site.url}}/scripts/LudumDare40/index.html) if you have a keyboard!

Source Code available [HERE](https://github.com/Sewerbird/LudumDare40)
...
### Monday, 09:45 (T+47:45)

Awake and showered, and making lunch for work. I [the code on Github](https://github.com/Sewerbird/LudumDare40), but I won't bother making a downloadable executable this morning - alarm was set for 9, instead of 7 like I'd planned, so I must rush off to work. I figure I'll leave on-time at the end of the day, and spend the evening doing baked-in pathfinding and cobbling together some graphics for the tiles and pieces.

See you in 10 hours!

### Sunday, 26:00 (T+40:00)

Hitting the long-tail of productivity, but I've been getting some fun little things done.

- When the Cad sits down to the Table, both the Date and Cad are engaged
- The Cad can Neg, Charm, or Talk, all of which affect the Date's irritation, hearts, and boredom
- The Cad can Disengage from a conversation, and thus leave the table to move freely
- The Cad uses the WASD/arrow keys for these actions in conversation
- The Date will become Super Hostile if they see the Cad with another Date. This is GAME OVER.
- The Date will Ditch The Date if irritation maxes out
- The Date will idle while alone at a table
- The Date has a high chance of Looking At The Phone. While doing so, the Date has zero line-of-sight
- The Hearts of a Date can now be Sad Hearts if negged excessively. This is a quick way to 'win' a Date, but lowers your score
- Dates who reach maximum Hearts, Sad or Happy, will begin to Go Home

As nice as all that is, I don't have Level Completed, Game Over, or logic to time the arrival of new dates, nor pathfinding for the dates to leave the building or berate the Cad. **Thus, I think I've failed the Compo.** As sad as that is, I think it's more from over-scoping and forcing myself to handmake everything above the OS and OpenGL layer myself. I'll resume tomorrow evening after work and see about submitting this effort to the *JAM* category, which is 3 days, not 2.

Good night!

### Sunday, 21:45 (T:35:45)

Been doing lots of work on line-of-sight, and I've gotten it working! Dates will now constantly scan for the Cad. They do not yet determine if the Cad is with another Date, but that's coming up next.

- I revisited my pathfinding failure and salvaged a good line-of-sight algorithm
- I hacked the Dates' states so that they use the line-of-sight algorithm
- I added a debug display mode to show tiles in view of a Date towards the Cad
- I made tiles block vision if marked so
- Multiple Dates can be on the map at the same time now, after some refactoring.'

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum15.gif"
   alt="Pink and Red circles walk up the stairs to a table. The Red circle sits down, and the Pink circle snaps to the chair opposite."
   />
</figure>

### Sunday, 18:30 (T+32:30)

I've gotten the Cad to seat the Date when he sits on a chair!

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum14.gif"
   alt="Pink and Red circles walk up the stairs to a table. The Red circle sits down, and the Pink circle snaps to the chair opposite."
   />
</figure>

This actually took several new concepts before this would work.

- Add tag types to Tiles, so I can distinguish between a Chair and a Wall in the AI
- Pair Chairs in the map together, so the Date knows which chair to sit in when the Cad chooses a table
- Make Cad issue events that the Date can respond to, in order to know when to sit in a chair

Next will be having Dates see each other.

### Sunday, 17:00 (T+31:00)

Done sulking, I decided to ditch pathfinding and do something much simpler: just copy where the Cad was at last, and make the date warp to there. It gives the illusion of doing what I wanted to do, with much less work:

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum13o.gif"
   alt="Yellow cab drops off the beautiful lady, and the Cad walks over to her, stops, and then escorts her into the building"
   />
</figure>

Next is to finish the escorting, and set the Cad and the Date at a table.

### Sunday, 16:00 (T+30:00)

What an awful afternoon so far: pathfinding has been a total disaster to try to get going. I'm gonna leave Coderbunker, sulk all the way home, and then try to approach with new vim and vigor.

### Sunday, 13:45 (T+27:45)

Great: spent some time editing the level, doing some visual indicators for the Date, and added some background music support as well.

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum12.jpg"
   alt="Screenshot of code source, showing a lot of states and transitions for the Date"
   />
</figure>

Next will be getting to the 'follow the cad' code.

### Sunday, 13:00 (T+27:00)

Alright, back to work. Gonna make quick meters for the Dates to show their status, as well as a label for their current State. Then I'll adjust the floorplan so that the Cad can go out and meet the Date: I'll then try to implement some basic 'follow the Cad' code.

### Sunday, 12:45 (T+26:45)

I've successfully gotten the Date changing states: she enters the Taxi from work, then disembarks when she arrives. The states that she moves between are themselves devoid of any behaviour, so I hooked up their onUpdate code to adjust her hearts, irritation, and restlessness for when she's stood waiting on the curb.

It took a while to wrangle, because I had to build out engine support for the onUpdate, and there was several bugs in the library code. But it works! Now I'll take a quick lunch break.

### Sunday, 11:15 (T+25:15)

Yikes, the Date has a *lot* of states, each with a behaviour it'll need. It'll be so cool if I can get these all wired up, but will easily take a few hours:

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum11.jpg"
   alt="Screenshot of code source, showing a lot of states and transitions for the Date"
   />
</figure>

My main hope is in that not all the states are crucial, and they each are very simple looking.

### Sunday, 10:00 (T+24:00)

After a bike ride, some finagling to get the space open, and getting some tea, we hit Hour 24: one full day has passed! I'm a bit more hopeful than I was when I went to bed last night. I have a few thoughts this morning:

- All the game logic remaining is very heavily tied to implementing the Dates' behaviour. This is essentially a game AI task, so I should prioritize formalizing the 'behaviour' tree I did at the very beginning of the Dare.
- I'll need a pathfinding algorithm for the dates, even if it's a naive one.
- I need a way to queue move events. My library has an untested module for this, so this presents some technical risk.
- It's gonna be a feat to find assets for everything: I'm doubtful I can draw everything I need.

So I'll work as diligently as I can today until 4pm, and then make a judgement call: if the game logic is all done, I'll hustle and draw a picture of the map my hand and split it into tiles and then submit the game Monday morning in time for the Compo. However, if things are looking bleak, I'll adjust my pace and change my goal to submitting to the Jam, which would give an extra day.

### Sunday, 09:15 (T+23:15)

Awake and showered. Just need to pack up some stuff and I'm off to Coderbunker again!

### Saturday, 26:00 (T+16:00)

Getting the Date *to step out* of the Cab ended up being much more annoying that I anticipated, and my eyes are getting bleary. Took me an embarassing amount of time to discover that the scenegraph wasn't being updated to include the new Splat of the Date: up to this point I've not 'dynamically' added anything to the board yet, so ran into the growing pain there. 

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum10.gif"
   alt="A yellow dot zooms up to the curb, stops, and a pink dot appears. The yellow dot stops a moment before zooming off"
   />
</figure>

SO, as of 16 hours of coding in, the game isn't as far along as I'd like it to be. I've succeeded in making a tilemap and moving the main character simply, and some taxi behaviour, but all of the logic associated with Dates isn't even begun yet. I'm still at awful-programmer-art in terms of assets, and it'll be a feat to squeeze in graphical polish after all the game logic I'll have to fit in tomorrow. I'm a bit pessimisstic about pulling off a A+ finish, but we'll see.

I'm going to go to bed and catch a few hours' sleep.

### Saturday, 24:00 (T+14:00)

I've managed to make it so that whenever I press `n` on the Cad, it spawns a new Date and puts her in the Cab. I still need to have her actually 'step out' of the cab when the taxi reaches the curb, however.

### Saturday, 23:45 (T+13:45)

Fatigue is really starting to set in. I *really* want to get a bit more logic done today, but I think my brainpower is waning. I think at the top of the hour I'll switch to less taxing aspects of the game, since I might be starting to introduce bugs if I go too late coding. I've done some twiddling around on the internets just for a moment.

### Saturday, 23:15 (T+13:15)

<figure>
  <img
   src="{{site.url}}/{{page.assets}}/ludum9.gif"
   alt="A yellow dot quickly pulls to curb, then away. Another yellow dot then follows without pulling over."
   />
</figure>

Pace is still slackening, but I did get the Taxi to behave propery in terms of pulling over to and away from the sidewalk curb. I'll start tackling queued movements next: right now the Taxi relies on tags on the tile to direct itself where to go.

- Allow Taxi to have a Passenger
- Implement mapfile tile designations "tile tags"
- Designate a 'pull over' tile
- Designate a 'drop off' tile
- Designate a 'pull away' tile
- Designate a 'passenger alight' tile
- Have Taxi pull over to the curb, stop, then pull away if it has a Passenger

I lost 15 minutes getting a gif of the pullover done, though.
